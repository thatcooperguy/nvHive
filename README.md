...
flowchart LR
    subgraph INSTALL["Install"]
        PIP["pip install nvhive"]
        EXTRAS["Optional extras\n[vision] [browser] [serve] [all]"]
    end

    subgraph SETUP["First Run: nvh"]
        S1["Step 1: Hardware + Local AI\nDetect GPU → install Ollama\n→ pull text + vision models"]
        S2["Step 2: Provider Status\nOllama running ✓\nCloud keys?"]
        S3["Step 3: API Keys\nDesktop agent opens browser\nclipboard detection\nor manual paste"]
    end
...
flowchart TB
    ...
    subgraph INTENT["Intent Detection Layer"]
        NL["Natural Language\nMapping"]
        AD["Action Detector\nregex patterns"]
        TD["Task Detector\nimperative verbs"]
        TC["Task Classifier\nTF-IDF + 88 examples\n13 task types"]
    ...
    subgraph ROUTING["Smart Routing Engine"]
        ROUTER{"Provider Router\ncapability 40% · cost 30%\nlatency 20% · health 10%"}
        LEARN["Adaptive Learning\nEMA scores · 20 queries\nto fully data-driven"]
    ...
    subgraph MODES["Execution Modes"]
        ...
        ASK["Single LLM Query\nroute to best advisor"]
        CODE["Agentic Coding\nplan → execute\n→ review → QA"]
        COUNCIL["Council\n3+ advisors\nweighted synthesis"]
    end
    ...
    subgraph TOOLS["Tool Registry"]
        ...
        BROWSER["Browser Tools\nnavigate · screenshot\nfill_form · http_request"]
        SYSTEM["System Tools\nshell · open · process\nclipboard · file ops"]
        CODETOOLS["Code Tools\nread · write · search\nrun_code · web_search"]
    end

    subgraph PROVIDERS["AI Providers"]
        LOCAL["Local (Ollama)\nnemotron · llama3.3:70b\nllama3.2-vision\n$0 · private"]
        FREE["Free Cloud\nGroq · Google Gemini\nLLM7 · GitHub\n$0"]
        CLOUD["Cloud\nOpenAI · Anthropic\nMistral · Cohere\npay-per-token"]
    end

    subgraph INFRA["Infrastructure"]
        OLLAMA["Ollama Server\nauto-install · auto-start\n~/.nvh/bin/ollama\nCUDA libs bundled"]
        CONFIG["Config\n~/.hive/config.yaml\nkeyring · .env\nper-profile"]
    ...
    style USER fill:#1a1a2e,stroke:#76B900,color:#e0e0e0
    style INTENT fill:#1a1a2e,stroke:#58a6ff,color:#e0e0e0
    style ROUTING fill:#1a1a2e,stroke:#e5c07b,color:#e0e0e0
    style MODES fill:#1a1a2e,stroke:#76B900,color:#e0e0e0
    style TOOLS fill:#1a1a2e,stroke:#58a6ff,color:#e0e0e0
    style PROVIDERS fill:#1a1a2e,stroke:#3fb950,color:#e0e0e0
    style INFRA fill:#1a1a2e,stroke:#8b949e,color:#e0e0e0
    style HW fill:#1a1a2e,stroke:#e5c07b,color:#e0e0e0
```
