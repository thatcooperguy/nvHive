"""SiliconFlow provider adapter via LiteLLM.

Uses the OpenAI-compatible SiliconFlow inference API:
  https://api.siliconflow.cn/v1

Permanently free models available at 1000 RPM — the highest rate limits
of any free provider. Large catalog of open-source models including
Qwen, DeepSeek, Llama, and many others.

Default model: Qwen/Qwen2.5-7B-Instruct (permanently free)
...
def _map_error(e: Exception, provider: str) -> ProviderError:
    """Map LiteLLM/SiliconFlow exceptions to our error taxonomy with actionable messages."""
...
def _calc_cost(model: str, usage: Usage) -> Decimal:
    """Permanently free models return zero cost."""
    return Decimal("0")


class SiliconFlowProvider:
    """SiliconFlow adapter using LiteLLM with OpenAI-compatible API."""
    ...
    def __init__(
        ...
        self._api_key = (
            ...
            or os.environ.get("HIVE_SILICONFLOW_API_KEY")
            or ""
        )
