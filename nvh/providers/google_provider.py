"""Google Gemini provider adapter via LiteLLM."""
...
class GoogleProvider:
    """Google Gemini adapter using LiteLLM."""
    ...
    async def complete(
        ...
        **kwargs: Any,
        ...
        finish_map = {"stop": FinishReason.STOP, "length": FinishReason.LENGTH}
        ...
        try:
            async for chunk in response:
                ...
                if chunk.choices and chunk.choices[0].finish_reason:
                    ...
                    finish_reason = {"stop": FinishReason.STOP, "length": FinishReason.LENGTH}.get(
                ...
                if is_final:
                    ...
                    usage = Usage(output_tokens=est_out, total_tokens=est_out)
    ...
    async def list_models(self) -> list[ModelInfo]:
        return [ModelInfo(model_id=self._default_model, provider=self._provider_name)]
    ...
    async def health_check(self) -> HealthStatus:
        ...
        try:
            ...
            elapsed = int((time.monotonic() - start) * 1000)
            return HealthStatus(provider=self._provider_name, healthy=True, latency_ms=elapsed)
        except Exception as e:
            elapsed = int((time.monotonic() - start) * 1000)
            return HealthStatus(
                provider=self._provider_name, healthy=False, latency_ms=elapsed, error=str(e)[:200]
            )
